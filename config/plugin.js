...
const svgToTinyDataUri = (() => {
  ...
  svgToTinyDataUri.toSrcset = (svg) =>
    svgToTinyDataUri(svg).replace(/ /g, "%20");
...
export default plugin(function ({ addBase, addComponents, theme }) {
  addBase({
    [[
      ...
      "border-color": theme("colors.gray.500", colors.gray[300]),
  ...
  addComponents({
    ...
    ".batch-actions-dropdown-menu": {
      "@apply z-10 hidden min-w-[7rem] bg-white rounded-md shadow-lg ring-1 ring-black/5 focus:outline-none dark:bg-gray-700 py-1 text-sm text-gray-700 dark:text-gray-200":
        {},
    },
